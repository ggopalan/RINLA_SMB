...
#if you don't have the following R packages, you can install with install.packages and then include the package name
library(plyr)
library(gridExtra)
library(ggplot2)
library(lattice)
...
# - PRborder: a 2 column matrix of x and y coordinates of top border, followed by bottom border, followed by the left border,
#            followed by the right border, in that order.
# - x_vals: vector of x coordinates in the grid being modeled
# - y_vals: vector of y coordinates in the grid being modeled
# - coords: 2 column matrix of x and y coordinates of the locations of the observed mass balance data
...
# - elevation.prd: vector that contains the surface elevation values for the to-be-predicted x, y coordiantes
...
m1 <- inla.mesh.2d(coords,loc.domain=PRborder,max.edge=.1)
prmesh <- m1
